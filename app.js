(() => {
  ...
  function renderSidebarRecentList(items) {
    ...
    items.slice(0, MAX_RECENT_WORK).forEach((item) => {
      ...
      button.title = item.title || "새 업무 요청";
  ...
  async function saveRemoteConversationMessage(role, text, metadata = {}) {
    ...
    try {
      ...
      if (data?.session?.id) {
        ...
        if (!current?.titleAutoGenerated && !current?.titleManuallyEdited && data.session.title) {
          patch.title = data.session.title;
        }
  ...
  function createConversationTitle(text) {
    ...
    const compact = cleaned
      ...
      .replace(/\s+/g, " ")
      .trim();
    const title = compact || cleaned;
    return title.length > AUTO_TITLE_MAX_CHARS ? `${title.slice(0, AUTO_TITLE_MAX_CHARS - 1)}…` : title;
  }
