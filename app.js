(() => {
  ...
  const KNOWLEDGE_CARD_ICONS = {
    ...
    owner: '<svg aria-hidden="true" class="quick-action-svg" focusable="false" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.2"><circle cx="12" cy="8" r="3.3"></circle><path d="M5.5 19c1.3-3 3.5-4.5 6.5-4.5s5.2 1.5 6.5 4.5"></path></g></svg>',
      ...
      <button id="dsOneOpenButton" type="button" aria-label="DS ONE 업무 AI 새 탭 열기" title="DS ONE 업무 AI 새 탭 열기">
        <span class="ds-one-btn-icon" aria-hidden="true">
          <svg viewBox="0 0 64 64" focusable="false">
            <path d="M32 6.5 53.8 19.1v25.2L32 56.9 10.2 44.3V19.1L32 6.5Z"/>
            <path d="M32 17.4 44.4 24.5v14.4L32 46 19.6 38.9V24.5L32 17.4Z"/>
            <path d="M44.4 24.5 53.8 19.1"/>
            <path d="M19.6 38.9 10.2 44.3"/>
            <path d="M32 6.5v10.9"/>
          </svg>
        </span>
  ...
  function injectSearchAndDialogStyles() {
    ...
    style.textContent = `
      ...
      .ds-guide-side {
        ...
        background:
          radial-gradient(circle at 16% 0%, rgba(47, 111, 237, .13), transparent 34%),
          linear-gradient(180deg, #fbfdff 0%, #f4f8ff 100%);
        border-right: 1px solid rgba(205, 218, 238, .9);
      ...
      .ds-guide-side::before {
        ...
        inset: 0 auto 0 0;
        width: 3px;
        background: linear-gradient(180deg, rgba(125, 168, 255, .78), rgba(47, 111, 237, .68) 48%, rgba(83, 210, 177, .48));
        opacity: .62;
      }
      .ds-guide-brand {
        position: relative;
      ...
      .ds-guide-nav button {
        ...
        border: 1px solid rgba(216, 226, 240, .78);
        border-radius: 14px;
        cursor: pointer;
      ...
      .ds-guide-nav button:focus-visible {
        color: #123b82;
        background: #ffffff;
        border-color: rgba(177, 202, 242, .88);
        outline: none;
      }
      .ds-guide-nav button[aria-current="true"] {
        color: #123b82;
        background: linear-gradient(135deg, #eaf2ff, #ffffff);
        border-color: rgba(77, 130, 226, .34);
        box-shadow: 0 14px 30px rgba(47, 111, 237, .13), inset 3px 0 0 #2f6fed;
      }
  ...
  function hideUnusedActionCard(card) {
    if (!card) return;
    card.hidden = true;
    card.disabled = true;
    card.style.display = "none";
    card.dataset.featureDisabled = "true";
    card.dataset.featureTask = "";
    card.dataset.featureTemplate = "";
    card.dataset.featureAttach = "false";
    card.dataset.featureMode = currentFeature;
    card.dataset.recommended = "false";
  ...
  function normalizeRecommendedQuestions(data) {
    const candidates = [
      ...
      data?.message?.recommendedQuestions,
      data?.message?.recommended_questions,
      data?.choices?.[0]?.recommendedQuestions,
      data?.choices?.[0]?.recommended_questions,
      data?.choices?.[0]?.message?.recommendedQuestions,
      data?.choices?.[0]?.message?.recommended_questions,
      data?.result?.recommendedQuestions,
      data?.result?.recommended_questions,
      data?.result?.message?.recommendedQuestions,
      data?.result?.message?.recommended_questions,
      data?.result?.choices?.[0]?.recommendedQuestions,
      data?.result?.choices?.[0]?.recommended_questions,
      data?.result?.choices?.[0]?.message?.recommendedQuestions,
      data?.result?.choices?.[0]?.message?.recommended_questions,
      data?.response?.recommendedQuestions,
      data?.response?.recommended_questions,
      data?.response?.message?.recommendedQuestions,
      data?.response?.message?.recommended_questions,
      data?.response?.choices?.[0]?.recommendedQuestions,
      data?.response?.choices?.[0]?.recommended_questions,
      data?.response?.choices?.[0]?.message?.recommendedQuestions,
      data?.response?.choices?.[0]?.message?.recommended_questions,
      data?.raw?.choices?.[0]?.recommendedQuestions,
      data?.raw?.choices?.[0]?.recommended_questions,
      data?.raw?.choices?.[0]?.message?.recommendedQuestions,
      data?.raw?.choices?.[0]?.message?.recommended_questions,
      data?.raw?.data?.choices?.[0]?.recommendedQuestions,
      data?.raw?.data?.choices?.[0]?.recommended_questions,
      data?.raw?.data?.choices?.[0]?.message?.recommendedQuestions,
      data?.raw?.data?.choices?.[0]?.message?.recommended_questions,
    ];
    const normalized = [];
      ...
      <div class="ds-guide-screen ds-guide-screen-home" aria-hidden="true">
        <div class="ds-guide-mock">
          ...
          <div class="ds-guide-mock-actions${is("actions")}">
            ...
            <span class="ds-guide-mock-action">
              <span class="ds-guide-mock-action-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="8"></circle><path d="M4.5 12h15"></path><path d="M12 4c2.1 2.2 3.2 5 3.2 8s-1.1 5.8-3.2 8"></path><path d="M12 4c-2.1 2.2-3.2 5-3.2 8s1.1 5.8 3.2 8"></path></svg></span>
